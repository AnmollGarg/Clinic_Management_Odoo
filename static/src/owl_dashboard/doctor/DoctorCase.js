...
export class DoctorCase extends Component {
    static template = "clinic_management.DoctorCases";
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            for (const c of cases) {
                c.medicines = (await this.orm.read(
                    "clinic.case.medicines",
                    c.medicines_ids,
                    ["id", "product_id", "product_name", "product_quantity", "product_UoM", "product_description"]
                )) || [];
                c.problem_solutions = (await this.orm.read(
                    "clinic.case.problem.solution",
                    c.problem_solution_ids,
                    ["id", "problem", "solution"]
                )) || [];
            }
